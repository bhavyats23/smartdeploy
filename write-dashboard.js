...
const RENDER_CONFIG = {
  smartdeploy: {
    hookUrl: "https://api.render.com/deploy/srv-d7oec7iqqhas73fl9aog?key=IpYlf7lvGJc",
    serviceId: "srv-d7oec7iqqhas73fl9aog",
  },
};
    ...
    try {
      const response = await fetch("/api/deploy", {
        ...
        body: JSON.stringify({
          repoName: repo.name,
          repoUrl: repo.url,
          renderHookUrl: renderConfig.hookUrl || "",
          renderServiceId: renderConfig.serviceId || "",
        }),
      });
        ...
        const poll = setInterval(async () => {
          ...
          try {
            ...
            if (thisDep && thisDep.status === "success") {
              ...
              setDeployStatus((prev) => { const n = Object.assign({}, prev); n[repo.name] = "live"; n[repo.name + "_url"] = liveUrl; return n; });
              ...
              clearInterval(poll);
              addLog("Build failed! Check Jenkins at http://localhost:8080");
              setDeployingRepo(null);
              setPipelineStep(-1);
            } else if (attempts >= 20) {
              clearInterval(poll);
              addLog("Timeout - check Jenkins at http://localhost:8080");
              setDeployingRepo(null);
              setPipelineStep(-1);
            }
          } catch (err) {
      ...
      <div className="dashboard-main">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: "24px", alignItems: "start" }}>
          <div>
            ...
            {loadingRepos ? (<div className="loading-repos">Loading repos...</div>) : repos.length === 0 ? (<div clas